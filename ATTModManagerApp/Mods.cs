...
namespace ATTTModManagerNet.Installer
{
    public partial class UnityModManagerForm : Form
    {
        ...
        private void SaveAndInstallZipFiles(string[] files)
        {
            ...
            foreach (string filepath in files)
            {
                try
                {
                    ...
                    {
                        Log.Print($"Only zip files are allowed.");
                    }
                }
                catch (Exception ex)
                {
                    Log.Print(ex.Message);
                    Log.Print($"Error installing file '{Path.GetFileName(filepath)}'.");
                }
            }
        ...
        private void UninstallMod(string name)
        {
            ...
            {
                Log.Print("Create a folder called mod in the ATT game directory.");
                return;
            }
        ...
        private void InstallMod(ZipFile zip, bool reloadMods = true)
        {
            ...
            if (!Directory.Exists(modsPath))
            {
                Log.Print("Install the ATTTModManager.");
                return;
            }

...
namespace ATTTModManagerNet.Installer
{
    ...
    {
        const string REG_PATH = @"HKEY_CURRENT_USER\Software\ATTTModManager";
        ...
        static readonly string[] libraryFiles = new string[]
        {
            ...
            "dnlib.dll",
            "System.Xml.dll",
            nameof(ATTTModManager) + ".dll",
            nameof(ATTTModManager) + ".xml"
        };
        ...
        private void RefreshForm()
        {
            ...
            managerPath = Path.Combine(managedPath, nameof(ATTTModManager));
            entryAssemblyPath = Path.Combine(managedPath, assemblyName);
            injectedEntryAssemblyPath = entryAssemblyPath;
            managerAssemblyPath = Path.Combine(managerPath, typeof(ATTTModManager).Module.Name);
            ...
            var managerType = typeof(ATTTModManager);
            ...
            if (managerInstalled != null && (hasInjectedAssembly || selectedGameParams.InstallType == InstallType.DoorstopProxy))
            {
                ...
                {
                    var versionString = managerInstalled.Fields.First(x => x.Name == nameof(ATTTModManager.version)).Constant.Value.ToString();
        ...
        //                        {
        //                            cryptoClasses.Add(new XElement("cryptoClass", new XAttribute("ummRngWrapper", "ATTTModManagerNet.RngWrapper, ATTTModManager, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null")));
        //                        }
        //                    }
                ...
                case Actions.Remove:
                    {
                        try
                        {
                            ...
                            if (v0_12_Installed != null || newWayInstalled != null)
                            {
                                ...
                                {
                                    instr = OpCodes.Call.ToInstruction(v0_12_Installed.Methods.First(x => x.Name == nameof(ATTTModManager.Start)));
                                }
        ...
        //{
        //    return doc.Root.Element("mscorlib").Attribute(nameof(ATTTModManager)) != null;
        //}

        //private static void MakeDirty(XDocument doc)
        //{
        //    doc.Root.Element("mscorlib").SetAttributeValue(nameof(ATTTModManager), ATTTModManager.version);
        //}

        private static bool IsDirty(ModuleDefMD assembly)
        {
            return assembly.Types.FirstOrDefault(x => x.FullName == typeof(Marks.IsDirty).FullName || x.Name == typeof(ATTTModManager).Name) != null;
        }

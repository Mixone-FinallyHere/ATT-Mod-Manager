...
namespace ATTTModManagerNet.Installer
{
    ...
    {
        readonly Dictionary<GameInfo, HashSet<ATTTModManager.Repository.Release>> repositories = new Dictionary<GameInfo, HashSet<ATTTModManager.Repository.Release>>();
        ...
        private void CheckModUpdates()
        {
            ...
            if (!repositories.ContainsKey(selectedGame))
                repositories.Add(selectedGame, new HashSet<ATTTModManager.Repository.Release>());
        ...
        private void ModUpdates_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e, GameInfo game, string url)
        {
            ...
            if (!e.Cancelled && !string.IsNullOrEmpty(e.Result) && repositories.ContainsKey(game))
            {
                try
                {
                    var repository = JsonConvert.DeserializeObject<ATTTModManager.Repository>(e.Result);
        ...
        private void LastVersion_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            ...
            if (!e.Cancelled && !string.IsNullOrEmpty(e.Result))
            {
                try
                {
                    ...
                    var release = repository.Releases.FirstOrDefault(x => x.Id == nameof(ATTTModManager));
                }
                catch (Exception ex)
                {
                    Log.Print(ex.Message);

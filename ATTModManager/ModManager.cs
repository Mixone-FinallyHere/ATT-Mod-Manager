...
namespace ATTTModManagerNet
{
    public partial class ATTTModManager
    {
        ...
        /// </summary>
        public static Version version { get; private set; } = typeof(ATTTModManager).Assembly.GetName().Version;

        private static ModuleDefMD thisModuleDef = ModuleDefMD.Load(typeof(ATTTModManager).Module);
        ...
        public partial class ModEntry
        {
            ...
            /// <summary>
            /// Displayed in UI. Add <color></color> tag to change colors. Can be used when custom verification game version [0.15.0]
            /// </summary>
                ...
                if (File.Exists(assemblyPath))
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            //if (modDef.TryGetTypeReference("UnityModManagerNet.ATTTModManager", out var typeRef))
                            //{
                            //    var managerAsmRef = new AssemblyNameReference("ATTTModManager", version);
                            ...
                            if (!cacheExists)
                            {
                                ...
                                {
                                    if (item.FullName == "UnityModManagerNet.ATTTModManager")
                                    {
            ...
            MethodInfo FindMethod(string namespaceClassnameMethodname, Type[] types, bool showLog = true)
            {
                ...
                if (!mCache.TryGetValue(key, out var methodInfo))
                {
                    ...
                    {
                        if (showLog)
                            ATTTModManager.Logger.Error($"Can't find method '{namespaceClassnameMethodname}'. Mod '{Info.Id}' is not loaded.");
                    }
        ...
        private static Assembly CurrentDomain_AssemblyResolve(object sender, ResolveEventArgs args)
        {
            ...
            if (args.Name.StartsWith("0Harmony,"))
            {
                ...
                if (match.Success)
                {
                    ...
                    string filepath = Path.Combine(Path.GetDirectoryName(typeof(ATTTModManager).Assembly.Location), $"0Harmony-{ver}.dll");
                    if (File.Exists(filepath))
                    {
                        try
                        {

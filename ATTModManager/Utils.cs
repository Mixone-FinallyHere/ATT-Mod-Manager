            ...
            foreach (var f in fields)
            {
                ...
                if (f2 == null)
                {
                    if ((mask & CopyFieldMask.Matching) == 0)
                        ATTTModManager.Logger.Error($"Field '{typeof(T2).Name}.{a.Alias}' not found");
                    continue;
                }
                if (f.FieldType != f2.FieldType)
                {
                    ATTTModManager.Logger.Error($"Fields '{typeof(T1).Name}.{f.Name}' and '{typeof(T2).Name}.{f2.Name}' have different types");
                    continue;
                }
